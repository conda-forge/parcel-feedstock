About parcel-feedstock
======================

Feedstock license: [BSD-3-Clause](https://github.com/conda-forge/parcel-feedstock/blob/main/LICENSE.txt)
...
Once the `conda-forge` channel has been enabled, `parcel` can be installed with `conda`:
...
```
mamba install parcel
```

It is possible to list all of the versions of `parcel` available on your platform with `conda`:
...
```
mamba search parcel --channel conda-forge
```
...
mamba repoquery search parcel --channel conda-forge

# List packages depending on `parcel`:
mamba repoquery whoneeds parcel --channel conda-forge

# List dependencies of `parcel`:
mamba repoquery depends parcel --channel conda-forge
```
